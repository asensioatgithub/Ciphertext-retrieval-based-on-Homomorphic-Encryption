...
///////////////////////////////////////
//InstInt Instruct::KeytoInstByte(unsigned char *n,int num){
//  InstInt r;
//  for(int i=7;i>=0;i--){
//    r.b[i].bit = (n[0]>>i)%2;
//    cout<<r.b[i].bit<<" ";
...
//  }
//   for(int i=23;i>=16;i--){
//    r.b[i].bit = (n[2]>>i-16)%2;
//    cout<<r.b[i].bit<<" ";
//  }
//  for(int j=31;j>=24;j--){
//    r.b[j].bit = (n[3]>>(j-24))%2;
//    cout<<r.b[j].bit<<" ";
//  }
//  cout<<endl;
//  return r;
...
void Instruct::KeytoInstByte(unsigned char *n,int num, InstByte *KeyWord){
   for(int i=0;i<num;i++){
      for(int j=7;j>=0;j--){
        KeyWord[i].b[j].bit = (n[i]>>j)%2;
    //    cout<<KeyWord[i].b[j].bit<<" ";
      }
    //  cout<<endl;
   }
...
void Instruct::Encrypt(InstByte *x,int size){
...
//			globalLevel = globalLevel + level_qisLessThan_Byte;
...
InstBit **Instruct::search(InstByte *key,InstList *Inst_table,int length){
    InstBit **r = new InstBit*[length];

    for(int i=0;i<length;i++){
        r[i] = new InstBit[4];
        for(int j=0;j<4;j++){
          r[i][j] = isEqual(key[j],Inst_table[i].Inst_KeyWord[j]);
        }
    }
    return r;      
}
