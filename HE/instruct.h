...
struct InstNode{
   InstByte Inst_d_id;
   InstByte Inst_wight;
   struct InstNode * next=NULL;
};

struct InstList{
   InstByte *Inst_KeyWord = NULL;
   InstNode * in=NULL;
   InstNode * tail=NULL;
   int KeySize;
};
...
public:
  ...
  void KeytoInstByte(unsigned char *n,int num, InstByte *KeyWord); // conver Key to InstInt 
  void Encrypt(InstByte *x,int size);
  InstBit **search(InstByte *key,InstList *table,int length);
